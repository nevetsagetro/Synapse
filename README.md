...
**Importing highlights:** Plug in your Kindle, grab `My Clippings.txt` from the `documents` folder, and upload it on the Import tab. Re-uploading the same file later is safe — it only adds new highlights.

---

## Gemini (optional)

The app works fine without it. If you want semantic "related highlights" search or AI book recommendations:
...
- **Daily Spark** resurfaces one highlight per day so old books don't disappear into the void
- **Personal Thoughts** lets you write timestamped reflections on any highlight, saved locally
- **Insights** shows your highlighting activity, top authors, and books you've marked up most
...
Stack: FastAPI + SQLModel on the backend, React + Vite + Tailwind on the front. Embeddings are stored as JSON in SQLite and similarity is computed with numpy — no Chroma, no Pinecone, no infrastructure to manage.